...
--------------------------------------------------------------------------------
-- 4) [251008] 담당자 컬럼 추가
-- member_id(MemberId)는 작성자
-- instructor_id(InstructorId)는 시설 담당자(강사)
--------------------------------------------------------------------------------

-- 컬럼 생성
ALTER TABLE facility_tbl ADD instructor_id VARCHAR2(20);

-- 컬럼 주석
COMMENT ON COLUMN facility_tbl.instructor_id IS '시설 담당자(강사) 회원ID(FK→member_tbl.member_id)';

-- FK 제약조건 추가
ALTER TABLE facility_tbl
ADD CONSTRAINT fk_facility_instructor
...
SELECT
    ...
    f.member_id           AS "작성자ID",       -- [251008]기준으로 memberID와 InstructorId로 구분 변경함
    instructor_id         AS "담당자ID",
